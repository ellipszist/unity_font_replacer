...
_RASTER_SHADER_REQUIREMENTS: dict[str, tuple[str, frozenset[str]]] = {
    ...
    # Sprite and Custom Atlas consume full RGB atlas samples. They are kept as
    # explicit diagnostic fallbacks because an Alpha8 target does not preserve
    # the white RGB channels generated by make_sdf.py.
    "tmp_sprite": (
        "TextMeshPro/Sprite",
        frozenset(
            {
...
예시:
  ...
  %(prog)s --gamepath "C:/path/to/game" --list font_map.json
        ...
        unsafe_full_color_help = "위험한 테스트 전용: Alpha8 atlas에 RGB를 읽는 TMP Sprite/Bitmap Custom Atlas shader fallback 허용"
...
Examples:
  ...
  %(prog)s --gamepath "C:/path/to/game" --list font_map.json
        ...
        unsafe_full_color_help = "Unsafe testing only: allow TMP Sprite/Bitmap Custom Atlas shaders that read RGB from an Alpha8 atlas"
