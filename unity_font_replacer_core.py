...
def _apply_raster_texture_sampling_contract(texture: Any) -> bool:
    """Use the sampling metadata required by an antialiased base-level atlas."""
    changed = False
    texture_settings = getattr(texture, "m_TextureSettings", None)
    if texture_settings is None or not hasattr(texture_settings, "m_FilterMode"):
        raise RuntimeError("Texture2D has no writable filter-mode metadata.")
    if int(getattr(texture_settings, "m_FilterMode", -1) or 0) != 1:
        texture_settings.m_FilterMode = 1  # Bilinear
        changed = True
    if not any(hasattr(texture, field) for field in ("m_MipMap", "m_MipCount")):
        raise RuntimeError("Texture2D has no writable mip-level metadata.")
    for field_name, desired in (
        ...
        if not hasattr(texture, field_name):
            continue
        current = getattr(texture, field_name)
        if current != desired:
            setattr(texture, field_name, desired)
            changed = True
    return changed


def _apply_raster_rgba32_storage(texture: Any, image: Image.Image) -> None:
    """Store raster coverage as white RGBA so full-color shaders can tint it."""
    set_image = getattr(texture, "set_image", None)
    if not callable(set_image):
        raise RuntimeError("Texture2D has no writable image encoder.")
    ...
    try:
        rgba.putalpha(alpha)
        set_image(rgba, target_format=4, mipmap_count=1)  # RGBA32
    finally:
        rgba.close()
        if alpha is not image:
            alpha.close()
    ...
    try:
        ...
        width = int(getattr(texture, "m_Width", 0) or 0)
        height = int(getattr(texture, "m_Height", 0) or 0)
        image_data = bytes(getattr(texture, "image_data", b"") or b"")
    except Exception as exc:
        raise RuntimeError("Could not verify RGBA32 raster atlas storage.") from exc
    if texture_format != 4 or width <= 0 or height <= 0:
        raise RuntimeError("Raster atlas encoder did not produce RGBA32 metadata.")
    if len(image_data) != width * height * 4:
        raise RuntimeError("Raster atlas encoder produced an invalid RGBA32 payload size.")
...
_RASTER_SHADER_REQUIREMENTS: dict[str, tuple[str, frozenset[str]]] = {
    "tmp_bitmap": (
        "TextMeshPro/Bitmap",
        frozenset(
            {
    ...
    # Sprite and Custom Atlas consume full RGB atlas samples. Their explicit
    # fallback mode therefore switches raster texture storage to white RGBA32.
    "tmp_sprite": (
        "TextMeshPro/Sprite",
        frozenset(
            {
    ...
    # Unity's built-in raster-font shader is an explicit diagnostic fallback.
    # It has no TMP/UI mask contract and always passes the depth test.
    "gui_text": (
        "GUI/Text Shader",
        frozenset({"_MainTex", "_Color"}),
    ),
}


def _material_shader_ref(material: Any) -> tuple[int, int] | None:
    shader_ref = getattr(material, "m_Shader", None)
    if shader_ref is None:
        return None
    if isinstance(shader_ref, dict):
        return (
            int(shader_ref.get("m_FileID", 0) or 0),
            int(shader_ref.get("m_PathID", 0) or 0),
        )
    return (
        int(getattr(shader_ref, "m_FileID", 0) or 0),
        int(getattr(shader_ref, "m_PathID", 0) or 0),
    )


def _set_material_shader_ref(material: Any, file_id: int, path_id: int) -> bool:
    shader_ref = getattr(material, "m_Shader", None)
    if shader_ref is None:
        return False
    desired = (int(file_id), int(path_id))
    current = _material_shader_ref(material)
    if isinstance(shader_ref, dict):
        shader_ref["m_FileID"], shader_ref["m_PathID"] = desired
    else:
        setattr(shader_ref, "m_FileID", desired[0])
        setattr(shader_ref, "m_PathID", desired[1])
    return current != desired


def _shader_name_and_properties(shader_reader: Any) -> tuple[str, frozenset[str]]:
    """Read the compiled Shader name and serialized property contract."""
    try:
        shader = shader_reader.parse_as_object()
    except Exception:
        return "", frozenset()
    parsed_form = getattr(shader, "m_ParsedForm", None)
    name = str(
        getattr(parsed_form, "m_Name", "")
        or getattr(shader, "m_Name", "")
        or ""
    )
    prop_info = getattr(parsed_form, "m_PropInfo", None)
    properties = frozenset(
        str(getattr(prop, "m_Name", "") or "")
        for prop in (getattr(prop_info, "m_Props", None) or [])
        if str(getattr(prop, "m_Name", "") or "")
    )
    return name, properties


def _classify_compatible_raster_shader(
    shader_name: str,
    shader_properties: Iterable[str],
) -> str | None:
    properties = frozenset(str(value) for value in shader_properties)
    for kind, (expected_name, required_properties) in _RASTER_SHADER_REQUIREMENTS.items():
...
def _raster_shader_descriptor(shader_reader: Any) -> JsonDict | None:
    if getattr(getattr(shader_reader, "type", None), "name", None) != "Shader":
        return None
    shader_name, shader_properties = _shader_name_and_properties(shader_reader)
    kind = _classify_compatible_raster_shader(shader_name, shader_properties)
    if kind is None:
        return None
    return {
        "kind": kind,
        "name": shader_name,
        "properties": sorted(shader_properties),
        "path_id": int(shader_reader.path_id),
        "assets_name": str(shader_reader.assets_file.name),
        "limited_masking": kind == "gui_text",
    }


def _scan_raster_shaders_in_outer_file(
    outer_file_key: str,
    target_assets_name: str,
    asset_file_index: dict[str, Any] | None,
) -> list[JsonDict]:
    """Scan one already-addressable outer file and cache pure shader metadata."""
    if not isinstance(asset_file_index, dict):
        return []
    normalized_outer = _normalize_asset_file_key(outer_file_key) or str(outer_file_key)
    cache = cast(
        dict[str, list[JsonDict]],
        asset_file_index.setdefault("raster_shader_scan_cache", {}),
    )
    cache_key = f"{normalized_outer}|{str(target_assets_name).lower()}"
    if cache_key in cache:
        return [dict(value) for value in cache[cache_key]]

    path_by_key = cast(dict[str, str], asset_file_index.get("path_by_key", {}))
    source_path = path_by_key.get(normalized_outer)
    if source_path is None and os.path.isfile(str(outer_file_key)):
        source_path = str(outer_file_key)
    descriptors: list[JsonDict] = []
    env = None
    try:
        if source_path is None:
            return []
        env = load_unitypy(source_path)
        for reader in env.objects:
            if str(reader.assets_file.name).lower() != str(target_assets_name).lower():
                continue
            descriptor = _raster_shader_descriptor(reader)
            if descriptor is not None:
                descriptors.append(descriptor)
    except Exception as exc:
        _log_debug(
            f"[raster_shader] scan failed: outer={source_path or outer_file_key} "
            f"assets={target_assets_name} error={type(exc).__name__}: {exc}"
        )
    finally:
        close_unitypy_env(env)
        env = None
        gc.collect()
    cache[cache_key] = [dict(value) for value in descriptors]
    return [dict(value) for value in descriptors]


def _resolve_compatible_raster_shader(
    source_assets_file: Any,
    material: Any,
    current_file_key: str,
    *,
    ...
    """Find a compatible Shader through PPtr routes the Material can address."""
    candidates: list[JsonDict] = []
    local_objects = getattr(source_assets_file, "objects", None)
    if isinstance(local_objects, dict):
        for reader in local_objects.values():
            descriptor = _raster_shader_descriptor(reader)
            if descriptor is None:
                continue
            descriptor["file_id"] = 0
            descriptor["outer_file_key"] = str(current_file_key)
    ...
    if isinstance(externals, (list, tuple)):
        for file_id in range(1, len(externals) + 1):
            target_assets_name = _resolve_assets_name_from_file_id(
                source_assets_file,
                file_id,
            )
            if not target_assets_name:
            ...
            if not target_outer_key:
                continue
            for descriptor in _scan_raster_shaders_in_outer_file(
                target_outer_key,
                target_assets_name,
                asset_file_index,
            ):
                descriptor["file_id"] = file_id
                descriptor["outer_file_key"] = str(target_outer_key)
                candidates.append(descriptor)

    current_shader_name = ""
    current_shader_ref = _material_shader_ref(material)
    shader_ref = getattr(material, "m_Shader", None)
    if shader_ref is not None:
        try:
            current_reader = shader_ref.deref()
            current_descriptor = _raster_shader_descriptor(current_reader)
            current_shader_name, _ = _shader_name_and_properties(current_reader)
            if current_descriptor is not None and current_shader_ref is not None:
                current_descriptor["file_id"] = int(current_shader_ref[0])
                current_descriptor["path_id"] = int(current_shader_ref[1])
                current_descriptor["outer_file_key"] = str(current_file_key)
                candidates.append(current_descriptor)
        except Exception:
            pass

    unsafe_full_color_kinds = {"tmp_bitmap_custom_atlas", "tmp_sprite"}
    if not allow_unsafe_full_color_shader_fallback:
        candidates = [
            value
            for value in candidates
            if str(value.get("kind", "")) not in unsafe_full_color_kinds
        ]
    if not allow_unsafe_gui_text_fallback:
        candidates = [
            value for value in candidates if str(value.get("kind", "")) != "gui_text"
        ]
    if not candidates:
        return None

    if current_shader_ref is not None:
        has_dedicated_bitmap = any(
            str(candidate.get("kind", ""))
            in {"tmp_bitmap", "tmp_mobile_bitmap"}
            for candidate in candidates
        )
        for candidate in candidates:
            candidate_ref = (
                int(candidate.get("file_id", 0) or 0),
                int(candidate.get("path_id", 0) or 0),
            )
            candidate_kind = str(candidate.get("kind", ""))
            if candidate_ref == current_shader_ref and (
                candidate_kind in {"tmp_bitmap", "tmp_mobile_bitmap"}
                or not has_dedicated_bitmap
            ):
                return candidate

    mobile_source = "/Mobile/" in current_shader_name
    if mobile_source:
        priority = {
            "tmp_mobile_bitmap": 0,
            "tmp_bitmap": 1,
            "tmp_bitmap_custom_atlas": 2,
            "tmp_sprite": 3,
            "gui_text": 4,
        }
    else:
    ...
    candidates.sort(
        key=lambda value: (
            priority.get(str(value.get("kind", "")), 99),
            int(value.get("file_id", 0) or 0) != 0,
            int(value.get("path_id", 0) or 0),
        )
    )
    return candidates[0]
...
def _prune_material_saved_properties_for_raster(
    ...
    *,
    ...
    supported_properties = {str(value) for value in shader_properties}
    expected = _RASTER_SHADER_REQUIREMENTS.get(shader_kind)
    if expected is None or not expected[1].issubset(supported_properties):
        return False

    tex_envs = getattr(saved_props, "m_TexEnvs", None)
    texture_map: dict[str, JsonDict] = {}
    if isinstance(tex_envs, list):
        for entry in tex_envs:
            if not isinstance(entry, (list, tuple)) or len(entry) < 2:
                continue
            prop_name = str(entry[0])
            texture_map[prop_name] = _extract_texture_ref_from_tex_env(entry[1])

    null_texture: JsonDict = {"m_FileID": 0, "m_PathID": 0}
    new_tex_envs: list[tuple[str, JsonDict]] = []
    for property_name in ("_FaceTex", "_MainTex"):
        if property_name not in supported_properties:
            continue
        new_tex_envs.append(
            (
                property_name,
                _build_tex_env_entry(texture_map.get(property_name, null_texture)),
            )
        )

    float_map: dict[str, float] = {}
    old_floats = getattr(saved_props, "m_Floats", None)
    if isinstance(old_floats, list):
        for entry in old_floats:
            if not isinstance(entry, (list, tuple)) or len(entry) < 2:
                continue
            try:
                value = float(entry[1])
    ...
    new_floats = [
        (
            property_name,
            default_value
            if property_name == "_Padding"
            else float_map.get(property_name, default_value),
        )
        for property_name, default_value in float_defaults.items()
    ...
    source_face_color = color_map.get("_FaceColor", color_map.get("_Color"))
    face_color_value = _color_value_to_dict(
        source_face_color,
        {"r": 1.0, "g": 1.0, "b": 1.0, "a": 1.0},
    )
    face_override = color_overrides.get("_FaceColor", color_overrides.get("_Color"))
    ...
    if "_ClipRect" in supported_properties:
        new_colors.append(("_ClipRect", clip_rect))
    if "_FaceColor" in supported_properties:
        new_colors.append(("_FaceColor", face_color_value))
    elif "_Color" in supported_properties:
        new_colors.append(("_Color", face_color_value))
...
def _reset_raster_material_keywords(
    material: Any,
    shader_properties: Iterable[str],
) -> bool:
    """Remove SDF keywords while rebuilding the raster alpha-clip toggle."""
    supported_properties = {str(value) for value in shader_properties}
    alpha_clip_enabled = False
    saved_props = getattr(material, "m_SavedProperties", None)
    float_props = getattr(saved_props, "m_Floats", None)
    if "_UseUIAlphaClip" in supported_properties and isinstance(float_props, list):
        for entry in float_props:
            if not isinstance(entry, (list, tuple)) or len(entry) < 2:
                continue
            if str(entry[0]) != "_UseUIAlphaClip":
                continue
            try:
                alpha_clip_enabled = abs(float(entry[1])) > 1e-9
            except (TypeError, ValueError, OverflowError):
                alpha_clip_enabled = False
            break

    desired_keywords = ["UNITY_UI_ALPHACLIP"] if alpha_clip_enabled else []
    changed = False
    if hasattr(material, "m_ShaderKeywords"):
        current = getattr(material, "m_ShaderKeywords", None)
        desired: Any
        if isinstance(current, str):
            desired = " ".join(desired_keywords)
        elif isinstance(current, (list, tuple, set)):
            desired = list(desired_keywords)
        else:
            desired = " ".join(desired_keywords)
        if current != desired:
            material.m_ShaderKeywords = desired
            changed = True
    if hasattr(material, "m_ValidKeywords"):
        current = getattr(material, "m_ValidKeywords", None)
        if current != desired_keywords:
            material.m_ValidKeywords = list(desired_keywords)
            changed = True
    if hasattr(material, "m_InvalidKeywords"):
        current = getattr(material, "m_InvalidKeywords", None)
        if current != []:
...
def _build_sdf_material_payload(
    *,
    ...
    # A raster shader has a different serialized property contract regardless
    # of whether the caller asked to keep the game's SDF styling. Preserve the
    # compatible color/stencil values, but always rebuild and retarget it.
    prune_raster_material = bool(force_raster)
...
def _select_tmp_raster_render_modes(
    data: JsonDict,
    generated_render_mode: int,
) -> tuple[int, int]:
    """Select runtime and creation-setting bitmap enums from serialized shape."""
    atlas_render_mode = int(generated_render_mode)
    if "m_AtlasRenderMode" in data:
        try:
            current_atlas_mode = int(data.get("m_AtlasRenderMode", 0) or 0)
        except (TypeError, ValueError, OverflowError):
            current_atlas_mode = generated_render_mode
        # Pre-TextCore TMP serialized RenderModes (0..3 bitmap, 6/7 SDF).
        if 0 <= current_atlas_mode < 0x100:
            atlas_render_mode = 0  # HintedSmooth

    creation_render_mode = atlas_render_mode
    creation_key = _resolve_creation_settings_key(data)
    creation_settings = data.get(creation_key) if creation_key else None
    if isinstance(creation_settings, dict):
        for key, value in creation_settings.items():
            if not key.replace("_", "").lower().endswith("rendermode"):
                continue
            try:
                current_creation_mode = int(value)
            except (TypeError, ValueError, OverflowError):
                continue
            if 0 <= current_creation_mode < 0x100:
                creation_render_mode = 0  # legacy HintedSmooth
            else:
                creation_render_mode = int(generated_render_mode)
            break
...
def _apply_material_replacement_to_object(parse_dict: Any, mat_info: JsonDict) -> bool:
    ...
    if prune_raster_material:
        ...
        shader_properties = mat_info.get("raster_shader_properties", [])
        raster_shader_properties = {
            str(value)
            for value in (
                shader_properties
                if isinstance(shader_properties, (list, tuple, set, frozenset))
                else []
            )
        }
        if not _prune_material_saved_properties_for_raster(
            parse_dict,
            color_overrides,
            shader_kind=shader_kind,
            shader_properties=raster_shader_properties,
        ):
            raise RuntimeError(
                "Could not rebuild Material SavedProperties for the selected "
                "raster shader contract."
            )
        changed = True
    else:
        float_props = getattr(saved_props, "m_Floats", None)
    ...
    if bool(mat_info.get("reset_keywords", False)) and prune_raster_material:
        if _reset_raster_material_keywords(parse_dict, raster_shader_properties):
            changed = True
    elif bool(mat_info.get("reset_keywords", False)):
...
def select_replacement_asset_padding(
    ...
    *,
    ...
    if _resolve_ttf_source_path(str(replacement_font)) is None:
        return None
    # Bitmap shaders have no SDF effect margin, so inheriting a large SDF
    # padding wastes atlas space and reduces the generated point size. Five
    # transparent texels cover TMP's one-texel bitmap baseline plus its
    # optional four-texel extra-padding UV expansion.
    if prefer_raster:
        return _RASTER_ATLAS_PADDING
    ...
    for obj in env.objects:
        ...
        if obj.type.name == "MonoBehaviour" and replace_sdf:
            ...
            if replacement_font:
                ...
                effective_force_raster = bool(
                    force_raster
                    or replacement_force_raster
                    or target_render_family == "bitmap"
                )
                ...
                if target_render_family not in {"sdf", "bitmap"}:
                    raise ValueError(
                        "Could not prove the selected TMP FontAsset render family; "
                        "refusing an unsafe replacement."
                    )
                ...
                selected_padding_variant = (
                    ...
                    if prefer_builtin_padding_variants and not effective_force_raster
                    else None
                )
                ...
                if assets["sdf_data"] and assets["sdf_atlas"]:
                    if lang == "ko":
                        _log_console(
                            f"{'Raster' if effective_force_raster else 'SDF'} 폰트 교체: "
                            f"{assets_name} | {objname} | (PathID: {pathid}) -> {replacement_font}"
                        )
                    else:
                        _log_console(
                            f"{'Raster' if effective_force_raster else 'SDF'} font replaced: "
                            f"{assets_name} | {objname} | (PathID: {pathid}) -> {replacement_font}"
                        )
                    ...
                    expected_replacement_family = (
                        "bitmap" if effective_force_raster else "sdf"
                    )
                    if replacement_render_family != expected_replacement_family:
                        raise ValueError(
                            f"TMP FontAsset requires a {expected_replacement_family} "
                            "replacement atlas for the selected mode."
                        )
                    ...
                    creation_render_mode: int | None = None
                    if effective_force_raster:
                        (
                            serialized_render_mode,
                            creation_render_mode,
                        ) = _select_tmp_raster_render_modes(
                            parse_dict,
                            replacement_render_mode,
                        )
                    elif "m_AtlasRenderMode" in parse_dict:
                        try:
                        ...
                        creation_render_mode = serialized_render_mode
                    ...
                    if isinstance(target_creation_settings, dict):
                        ...
                        _sync_creation_settings_payload(
                            ...
                            render_mode=creation_render_mode,
                        )
                    ...
                    material_payload["expected_shader_family"] = target_render_family
    ...
    for obj in env.objects:
        ...
        if obj.type.name == "Texture2D":
            ...
            if isinstance(texture_plan, dict):
                ...
                raster_texture_metadata_changed = False
                raster_full_color_storage = parse_bool_flag(
                    texture_plan.get("raster_full_color_storage")
                )
                if parse_bool_flag(texture_plan.get("raster_texture_contract")):
                    raster_texture_metadata_changed = (
                        _apply_raster_texture_sampling_contract(parse_dict)
                    )
                    if (
                        raster_texture_metadata_changed
                        and (_has_trailing_bytes(obj) or typetree_size_mismatch)
                    ):
                        raise RuntimeError(
                            "Raster Texture2D needs Bilinear/base-level metadata, "
                            "but this object's extra-byte layout requires binary-only "
                            "saving. Refusing to leave incompatible sampling metadata."
                        )
                if raster_full_color_storage:
                    if ps5_swizzle:
                        raise RuntimeError(
                            "RGBA32 raster fallback storage is not implemented for "
                            "the PS5 swizzle path. Refusing an unverified layout."
                        )
                    if _has_trailing_bytes(obj) or typetree_size_mismatch:
                        raise RuntimeError(
                            "RGBA32 raster fallback storage must update Texture2D "
                            "format metadata, but this object's extra-byte layout "
                            "requires binary-only saving."
                        )
                    if not isinstance(replacement_image, Image.Image):
                        raise RuntimeError("RGBA32 raster atlas image is unavailable.")
                    _apply_raster_rgba32_storage(parse_dict, replacement_image)
                    applied_texture_image = True
                    _log_debug(
                        f"[replace_texture] file={fn_without_path} assets={assets_name} "
                        f"path_id={obj.path_id} action=rgba32_raster_injection "
                        f"raw_size={len(getattr(parse_dict, 'image_data', b'') or b'')}"
                    )
                    if lang == "ko":
                        _log_console(
                            "  Full-color shader용 RGBA32 raster atlas를 적용했습니다."
                        )
                    else:
                        _log_console(
                            "  Applied an RGBA32 raster atlas for full-color shader sampling."
                        )
                if (
                    not applied_texture_image
        ...
        if obj.type.name == "Material":
            ...
            if mat_info is not None:
                ...
                effective_mat_info = mat_info
                material_shader_rebound = False
                if bool(mat_info.get("retarget_raster_shader", False)):
                    raster_shader = _resolve_compatible_raster_shader(
                        obj.assets_file,
                        parse_dict,
                        current_file_key,
                        source_bundle_signature=source_bundle_signature,
                        ...
                        allow_unsafe_gui_text_fallback=allow_unsafe_gui_text_fallback,
                    )
                    if raster_shader is None:
                        raise RuntimeError(
                            "No compatible raster shader is reachable from this "
                            "Material. The build must contain TextMeshPro/Bitmap or "
                            "TextMeshPro/Mobile/Bitmap through an existing PPtr route. "
                            "Full-color Sprite/Custom Atlas and basic GUI/Text fallbacks "
                            "can only be enabled explicitly with their unsafe testing "
                            "options."
                        )
                    desired_shader_ref = (
                        int(raster_shader.get("file_id", 0) or 0),
                        int(raster_shader.get("path_id", 0) or 0),
                    )
                    if desired_shader_ref[1] == 0:
                        raise RuntimeError(
                            "Compatible raster shader resolved to a null PathID."
                        )
                    if _material_shader_ref(parse_dict) != desired_shader_ref:
                        material_shader_rebound = _set_material_shader_ref(
                            parse_dict,
                            *desired_shader_ref,
                        )
                    if _material_shader_ref(parse_dict) != desired_shader_ref:
                        raise RuntimeError(
                            "Material.m_Shader raster retarget verification failed."
                        )
                    effective_mat_info = dict(mat_info)
                    effective_mat_info["raster_shader_kind"] = str(
                        raster_shader.get("kind", "")
                    )
                    effective_mat_info["raster_shader_properties"] = list(
                        raster_shader.get("properties", [])
                    )
                    _log_debug(
                        f"[raster_shader] file={fn_without_path} assets={assets_name} "
                        f"material_path_id={obj.path_id} shader={raster_shader.get('name')} "
                        f"shader_ref={desired_shader_ref[0]}:{desired_shader_ref[1]} "
                        f"kind={raster_shader.get('kind')}"
                    )
                    if bool(raster_shader.get("limited_masking", False)):
                        _log_warning(
                            f"[raster_shader] Material {assets_name}|{obj.path_id} "
                            "uses GUI/Text Shader because this build stripped all "
                            "TMP bitmap-compatible UI shaders; stencil and RectMask2D "
...
Examples:
  ...
  %(prog)s --gamepath "C:/path/to/game" --list font_map.json
    ...
    parser.add_argument(
        "--allow-unsafe-full-color-shader-fallback",
        action="store_true",
        help=unsafe_full_color_help,
    )
    parser.add_argument(
        "--allow-unsafe-gui-text-fallback",
        action="store_true",
        help=unsafe_gui_text_help,
    )
    parser.add_argument(
