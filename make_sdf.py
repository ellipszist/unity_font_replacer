...
def _parse_point_size(value: str) -> int:
    ...
    try:
        ...
        raise argparse.ArgumentTypeError("point size must be an integer or 'auto'.") from exc
    if point_size <= 0:
...
def _resolve_ttf_path(ttf_arg: str) -> Path:
    ...
    base_dirs = [Path.cwd(), Path(__file__).resolve().parent, Path.cwd() / "KR_ASSETS", Path(__file__).resolve().parent / "KR_ASSETS"]
...
def _load_charset_text(charset_arg: str) -> str:
    ...
    looks_like_path = any(sep in charset_arg for sep in ["/", "\\"]) or charset_arg.lower().endswith(".txt")
    if looks_like_path:
...
def _pack_rectangles_shelf(
    ...
    y = 0
    row_height = 0
    ordered = sorted(rectangles, key=lambda item: (item[1] * item[2], item[2], item[1]), reverse=True)
    ...
    if isinstance(data.get("m_AtlasTextures"), list):
        ...
        for tex in data["m_AtlasTextures"]:
            if isinstance(tex, dict):
                normalized.append({"m_FileID": int(tex.get("m_FileID", 0) or 0), "m_PathID": int(tex.get("m_PathID", 0) or 0)})
...
def generate_sdf_assets_from_ttf(
    ...
    if np is None or scipy_ndimage is None:
        print("Error: numpy/scipy are required.")
        return None
    if not unicodes:
        return None
    ...
    def _build_layout(candidate_point_size: int) -> JsonDict | None:
        ...
        for code in unicodes:
            glyph_w, glyph_h, metrics = _measure_glyph_metrics(font, int(code), int(ascent))
    ...
    if requested_point_size <= 0:
        ...
        for candidate in candidates:
            ...
            _emit(f"[fixed] point-size {candidate}: {'fit' if layout is not None else 'overflow'}")
            if layout is not None:
    ...
    for idx, entry in enumerate(selected_entries, start=1):
        ...
        if glyph_w > 0 and glyph_h > 0:
            ...
            if copy_w > 0 and copy_h > 0:
                tile[offset_y:offset_y + copy_h, offset_x:offset_x + copy_w] = bitmap[:copy_h, :copy_w]

            if normalized_render_mode == "sdf":
                mode_tile = _compute_sdf_tile(tile, atlas_padding)
            else:
                mode_tile = tile
            atlas_alpha[py:py + ph, px:px + pw] = np.maximum(atlas_alpha[py:py + ph, px:px + pw], mode_tile)
        ...
        char_table.append({"m_ElementType": 1, "m_Unicode": unicode_value, "m_GlyphIndex": glyph_index, "m_Scale": 1.0})
    ...
    cap_line = float(selected_ascent - cap_bbox[1]) if cap_bbox is not None else float(selected_ascent)
    mean_line = float(selected_ascent - mean_bbox[1]) if mean_bbox is not None else float(selected_ascent * 0.5)
    ...
    strikethrough_offset = cap_line / 2.5 if cap_line != 0 else float(selected_ascent) * 0.4
    ...
    gradient_scale = float(atlas_padding + 1) if normalized_render_mode == "sdf" else 1.0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--atlas-size", default="4096,4096", help="Atlas size in 'W,H' format. Default: 4096,4096")
    parser.add_argument("--point-size", default="auto", help="Sampling point size or 'auto'. Default: auto")
    parser.add_argument("--padding", type=int, default=7, help="Atlas padding. Default: 7")
    parser.add_argument("--charset", default="./CharList_3911.txt", help="Charset file path or literal characters.")
    parser.add_argument("--rendermode", choices=["sdf", "raster"], default="sdf", help="Render mode. Default: sdf")
    return parser
...
def run_make_sdf(argv: list[str] | None = None) -> int:
    ...
    if generated is None:
        print("Error: failed to generate atlas/json from TTF.")
        return 1
    ...
    if not isinstance(sdf_data, dict) or atlas_image is None:
        print("Error: generator returned invalid payload.")
        return 1
    ...
    if material_path.exists():
        print(f"Generated: {material_path}")
    print(
        "Summary: "
