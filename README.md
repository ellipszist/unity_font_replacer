...
`Actions` → `Build and Release` → `Run workflow`에서 버전(예: `1.3.0` 또는 `v1.3.0`)을 입력합니다. 소스는 항상 체크아웃 시점의 기본 브랜치 최신 커밋을 사용합니다.

- 기존 버전: 같은 이름의 한국어·영어 UFR 및 `make_sdf` ZIP 3개만 교체합니다. 릴리즈 노트, 제목, 공개/초안·사전 릴리즈·Latest 상태, 기존 태그와 다른 첨부 파일은 유지됩니다.
- 새 버전: 자동 생성된 릴리즈 노트를 포함한 초안 릴리즈를 만듭니다.
- 입력 버전은 릴리즈 태그와 ZIP 이름에만 사용하며, 해당 버전의 과거 소스를 빌드하지 않습니다. 기존 태그는 이동하지 않으므로 GitHub의 `Source code` 다운로드는 원래 태그의 소스를 유지합니다. 실제 빌드 커밋은 Actions 실행 요약에 표시됩니다.
- 테스트·빌드·패키징이 성공한 뒤 업로드합니다. 첨부 파일 교체는 원자적이지 않으므로 업로드 도중 실패하면 일부 파일이 빠지거나 이전 파일과 섞일 수 있습니다. 같은 버전으로 다시 실행하면 교체를 재시도합니다. GitHub에서 변경 불가(immutable)로 잠긴 릴리즈는 교체할 수 없습니다.
