...
class SaveRegressionTests(unittest.TestCase):
    ...
    def test_material_reconciliation_completion_respects_patch_strength(self) -> None:
        class FakeType:
            name = "Material"
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            file_key = core._normalize_asset_file_key(str(target))
            object_key = core._make_assets_object_key("CAB-test", 7)
            atlas_key = core._make_assets_object_key("CAB-atlas", 19)
            material_identity = core._make_outer_assets_object_key(
                file_key,
                "CAB-test",
                7,
            )
            ...
            def fake_apply_material(parsed, payload) -> bool:
                applied_payloads.append(dict(payload))
                return True

            def fake_parse_material(obj):
                return SimpleNamespace(
            ...
            with (
                ...
                patch.object(
                    core,
                    "_safe_parse_as_object",
                    side_effect=fake_parse_material,
                ),
                patch.object(
                    core,
                    "_resolve_material_main_texture_identity",
                    return_value=atlas_key,
                ),
                patch.object(
                    core,
                    "_apply_material_replacement_to_object",
                ...
                direct_completed = {
                    core._material_patch_completion_key(
                        material_identity,
                        direct=True,
                    )
                }
                linked_payload = {"outline_ratio": 2.0}
                atlas_plans = {file_key: {atlas_key: linked_payload}}

                linked_result = core.replace_fonts_in_file(
                    "2021.3.0f1",
                    str(game_path),
                    str(target),
                    {},
                    replace_ttf=False,
                    replace_sdf=False,
                    temp_root_dir=str(root / "scratch-linked"),
                    generator=object(),
                    replacement_lookup={},
                    deferred_material_atlas_plans=atlas_plans,
                    patched_material_identities=direct_completed,
                    lang="en",
                )

                self.assertFalse(linked_result)
                self.assertEqual(applied_payloads, [])
                self.assertEqual(atlas_plans, {})
                ...
                direct_payload = {"outline_ratio": 2.0}
                direct_plans = {file_key: {object_key: direct_payload}}

                direct_result = core.replace_fonts_in_file(
                    "2021.3.0f1",
                    str(game_path),
                    str(target),
                    {},
                    replace_ttf=False,
                    replace_sdf=False,
                    temp_root_dir=str(root / "scratch-direct"),
                    generator=object(),
                    replacement_lookup={},
                    deferred_material_plans=direct_plans,
                    patched_material_identities=linked_completed,
                    lang="en",
                )

                srp_completed: set[str] = set()
                srp_linked_payload = {
                    "outline_ratio": 1.0,
                    "require_tmp_signature": True,
                }
                srp_atlas_plans = {file_key: {atlas_key: srp_linked_payload}}
                srp_result = core.replace_fonts_in_file(
                    "2021.3.0f1",
                    str(game_path),
                    str(target),
                    {},
                    replace_ttf=False,
                    replace_sdf=False,
                    temp_root_dir=str(root / "scratch-srp"),
                    generator=object(),
                    replacement_lookup={},
                    deferred_material_atlas_plans=srp_atlas_plans,
                    patched_material_identities=srp_completed,
                    lang="en",
                )

            self.assertTrue(direct_result)
            self.assertTrue(srp_result)
            self.assertEqual(len(applied_payloads), 2)
            self.assertTrue(
                applied_payloads[0]["game_outline_ratio_already_applied"]
            )
            self.assertEqual(direct_plans, {})
            self.assertEqual(srp_atlas_plans, {})
            self.assertIn(
                core._material_patch_completion_key(
                    material_identity,
                    direct=True,
